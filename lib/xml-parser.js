...
function _parseParams(innerText) {
  ...
  for (const tagName of tagNames) {
    ...
    if (rawStringParams.has(tagName)) {
      // For raw string params, find the LAST occurrence of the closing tag
      // This way, nested HTML inside content/command won't break parsing
      const allCloseMatches = [];
      let cm;
      const re = new RegExp(closeTagRe.source, 'gi');
      while ((cm = re.exec(innerText)) !== null) {
        allCloseMatches.push(cm);
      }
      if (allCloseMatches.length === 0) continue;
      closeMatch = allCloseMatches[allCloseMatches.length - 1];
    } else {
