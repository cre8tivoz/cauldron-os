...
(async () => {
  ...
  assert.match(
    ...
    'project-folder import button should exist in UI'
  );
  ...
  const dryRun = await request('/api/projects/import?dryRun=1', {
    method: 'POST',
    body: JSON.stringify({}),
  });
  ...
  assert.ok(
    dryRun.body.projects.some((project) => project.name === 'ghosted-social-club'),
    'dry-run should include Ghosted Social Club'
  );

  const importResult = await request('/api/projects/import', {
    method: 'POST',
    body: JSON.stringify({}),
  });
  ...
  assert.ok(
    importResult.body.imported + importResult.body.skipped >= 5,
    'project import should account for existing project folders'
  );

  const drafts = await request('/api/drafts?q=ghosted');
  assert.equal(drafts.res.status, 200, 'draft search should load');
  assert.ok(
    drafts.body.drafts.some((draft) => /ghosted/i.test(draft.project_name)),
    'imported Ghosted draft should be searchable'
  );
