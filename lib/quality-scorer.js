...
function findHexColors(html = '') {
  return Array.from(new Set(String(html).match(/#[0-9a-f]{3}(?:[0-9a-f]{3})?\b/gi) || [])).map(
...
function scoreAccessibility(html, text) {
  ...
  const labelledControls = countMatches(
    html,
...
function scoreSpacing(html) {
  let score = 35;
  const spacingRules = countMatches(
    html,
...
function scoreSemanticHtml(html) {
  let score = 25;
  const semanticTags = countMatches(
    html,
