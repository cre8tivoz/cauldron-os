...
- **URL Research Results Panel**: Scraped design variables (colors, typography) from the Reference URL are now immediately visible in the UI after generating a blueprint.
- **Refine Blueprint Engine**: Added `/api/refine` backend endpoint and a frontend UI input allowing users to iteratively tweak generated blueprints without starting from scratch.
- **Download Prototype Button**: A direct one-click button to download the raw HTML `prototype.html` artifact bypassing the manual copy/paste.
- **Blueprint Section Nav Pills**: Auto-extracts Markdown headers from the generated blueprint to create quick-jump scrollable navigation links.
- **Progress Stages**: Generation button now cycles through status text (Structuring architecture, Writing prototype, etc.) to indicate active processing.
- **Custom Project Naming**: If saving a draft/blueprint without entering a project name, Cauldron will now prompt you for a custom directory name rather than defaulting to `project-X`.
...
_This CHANGELOG will be updated with each release._
