...
assert.equal(
  npmSpawnOptions('C:\\cauldron-project', 'win32').shell,
  true,
  'Windows npm.cmd should run through the command shell'
);
assert.equal(
  npmSpawnOptions('/tmp/cauldron-project', 'linux').shell,
  false,
  'Unix npm should not require shell execution'
);
...
for (const [name, script] of [
  ...
  assert.match(
    script,
    /npm (?:ls|install|start)|npmCommand/,
    `${name} launcher should use the npm package contract`
  );
  assert.doesNotMatch(
    script,
    /2\.3\.0|OLLAMA_MODEL/,
    `${name} launcher should not contain stale version or model defaults`
  );
}
...
assert.match(
  ...
  'PowerShell launcher should run from its own directory'
);
assert.match(
  powershell,
  /\$nodeMajor -lt 18/,
  'PowerShell launcher should enforce the supported Node major version'
);
assert.match(readme, /Windows \(easiest\)/, 'README should identify the simplest Windows path');
assert.match(
  readme,
  /double-click `start-cauldron\.bat`/,
  'README should expose the double-click Windows launcher'
);
assert.match(
  ciWorkflow,
  /windows-smoke-test:[\s\S]*runs-on: windows-latest[\s\S]*npm test/,
  'CI should run the full smoke suite on Windows'
);
