...
const { assertSafeModelBaseUrl } = require('./url-safety');
...
async function normaliseOpenAICompatibleChatUrl(baseUrl = '') {
  ...
  const safe = await assertSafeModelBaseUrl(normalised, 'Model base URL');
...
async function callCloudModel({
  ...
  const url =
    provider === 'gemini' ? GEMINI_BASE_URL : await normaliseOpenAICompatibleChatUrl(baseUrl);
