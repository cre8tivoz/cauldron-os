 ...
 * Allows loopback and private LAN targets (local gateways), but blocks
 * link-local, cloud-metadata, and credentialed URLs.
 */
function assertSafeModelBaseUrl(rawUrl, label = 'Model base URL') {
  ...
  if (net.isIP(hostname) && isLinkLocalOrMetadataIp(hostname)) {
