...
const { assertSafeModelBaseUrl, fetchPinnedUrl } = require('./url-safety');
...
async function normaliseOpenAICompatibleChatUrl(baseUrl = '') {
  ...
  return assertSafeModelBaseUrl(normalised, 'Model base URL');
}
...
async function callCloudModel({
  ...
  try {
    ...
    if (provider === 'gemini') {
      ...
      response = await fetchPinnedUrl(target, {
        method: 'POST',
        headers,
        signal: controller.signal,
        body,
