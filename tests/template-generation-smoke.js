...
(async () => {
  ...
  try {
    ...
    assert.ok(
      ollamaPayload.system.includes('## Project Type: HTML + AlpineJS'),
      'system prompt should include selected template heading'
    );
    assert.ok(
      ollamaPayload.system.includes('AlpineJS state'),
      'system prompt should include template prompt bias'
    );
    assert.ok(
      ollamaPayload.system.includes('index.html'),
      'system prompt should include expected file names'
    );
    ...
    assert.strictEqual(
      historyData.sessions[0].generation_mode,
      'local',
      'session should record local generation mode'
    );
