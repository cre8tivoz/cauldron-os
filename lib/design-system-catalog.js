...
function discoverLocalDesignSystems({ rootDir, catalogPath = 'design-systems/catalog.json' }) {
  ...
  return fs
    .readdirSync(designSystemsDir, { withFileTypes: true })
...
function discoverCommunityDesignSystems({ rootDir }) {
  const dataRoot = process.env.CAULDRON_DATA_DIR || path.join(rootDir, 'data');
  const communityDir = path.join(dataRoot, 'community', 'design-systems');
  ...
  return fs
    ...
    .filter((entry) => entry.isDirectory() && !entry.name.startsWith('_'))
    .map((entry) => {
      const designPath = path.join(communityDir, entry.name, 'DESIGN.md');
      if (!fs.existsSync(designPath)) return null;
      const content = fs.readFileSync(designPath, 'utf8');
      const id = normaliseSystemId(entry.name);
      return {
        ...
        source: 'community',
        origin: 'runtime-community-import',
      };
    })
    .filter(Boolean)
    .sort((a, b) => a.name.localeCompare(b.name));
...
function createDesignSystems({ rootDir, legacySystems = {}, referoStyles = {} }) {
  ...
  for (const community of discoverCommunityDesignSystems({ rootDir })) {
    systems[community.id] = {
      name: community.name,
      repo: null,
      absolutePath: community.absolutePath,
...
function createDesignSystemService({ rootDir, systems, remoteBaseUrl }) {
  ...
  function fetchDesignSystem(systemOrRepo, callback) {
    ...
    if (system?.absolutePath) {
      fs.readFile(system.absolutePath, 'utf8', callback);
      return;
    }
  ...
  function ensureDesignSystem(systemId) {
    ...
    if (system.__refero && system.promptGuidance) {
      const content = [
        ...
        '',
        Array.isArray(system.colors) && system.colors.length
          ? `Colors: ${system.colors.join(', ')}`
          : '',
        Array.isArray(system.fonts) && system.fonts.length
...
module.exports = {
  ...
  discoverCommunityDesignSystems,
