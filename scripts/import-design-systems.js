...
const { extractDesignSystemName, normaliseSystemId } = require('../lib/design-system-catalog');
...
function extractFonts(content) {
  ...
  for (const match of String(content).matchAll(
    /(?:font|typeface|family|primary font|heading font)[^:\n]*:\s*([^\n]+)/gi
  )) {
...
async function main() {
  ...
  if (designFiles.length < MIN_EXPECTED) {
    throw new Error(
      `Expected at least ${MIN_EXPECTED} DESIGN.md files from ${SOURCE_REPO}, found ${designFiles.length}`
    );
  }
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, 'LICENSE-OPEN-DESIGN.txt'),
    await fetchText(`${RAW_BASE}/LICENSE`),
    'utf8'
  );
  fs.writeFileSync(
