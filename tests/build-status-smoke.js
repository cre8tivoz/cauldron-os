...
(async () => {
  ...
  assert.match(
    ...
    'Gemini Flash 3.1 Lite preview should be exposed in UI'
  );
  assert.match(
    home.text,
    /gemini-3\\.1-pro-preview/,
    'Gemini Pro 3.1 Preview should be exposed in UI'
  );
  ...
  assert.equal(
    models.body.gemini.defaultModel,
    'gemini-3.1-flash-lite-preview',
    'Gemini should default to Flash Lite preview'
  );
  assert.ok(
    models.body.gemini.models.includes('gemini-3.1-pro-preview'),
    'Gemini Pro preview should be accepted'
  );
  ...
  const dryResume = await request('/api/projects/funeral-mode/resume?dryRun=1', {
    method: 'POST',
    body: JSON.stringify({}),
  });
  ...
  const dryVisible = await request('/api/projects/funeral-mode/open-visible?dryRun=1', {
    method: 'POST',
    body: JSON.stringify({}),
  });
  ...
  assert.equal(
    buildStatus.body.status.sessionId,
    'smoke-test-session',
