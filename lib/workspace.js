...
async function wsWriteFile(sessionId, filePath, content) {
  ...
  // If the path is a symlink (including a dangling one), never write through it —
  // writeFile() follows links and could escape the workspace. Only write to a
  // confined realpath when the link resolves inside the sandbox.
  let writePath = target;
  try {
    const stat = await fs.promises.lstat(target);
    if (stat.isSymbolicLink()) {
      try {
        writePath = await _realpathInside(sessionId, target);
      } catch (err) {
        if (err && err.code === 'ESECURITY') throw err;
        throw _securityError('Security: Symlink target is not writable inside workspace');
      }
    }
  } catch (err) {
    if (err && err.code === 'ESECURITY') throw err;
